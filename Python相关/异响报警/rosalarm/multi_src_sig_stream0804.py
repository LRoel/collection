...
ny = len(y1) # 10s录音数据长度
N_step = int(math.floor(ny/pt_step)) # 10s录音中的能量点数

### 求所有能量点 & 求平均能量
# for循环，求10s内所有能量点
for i_step in range(0,N_step):# 循环从0开始，循环次数为 N_step
    abs_y1_smth = np.abs(y1[i_step * pt_step : (i_step + 1) * pt_step])
    eng_step.append(np.sum(abs_y1_smth))

# for循环，求平均能量
for i_avrg in range(0,int((math.floor(N_step / 5) - 1))):# 循环从0开始，循环次数为后面int结果
...
while (loop_sign == 0):# 循环在条件成立时执行，否则终止
    ...
    if alarm_sign == 0 :# 当前是非报警状态
        ...
        if (avrg_eng_step[n_avrg_eng_step] > (background_eng*2)):# 是报警点
        ...
        elif (avrg_eng_step[n_avrg_eng_step-1] > (background_eng*1.5)):# 是过渡点
            alarm_sign = -1# 过渡点
            ...
            background_eng = avrg_eng_step[n_avrg_eng_step-1]# 更新背景值
        ...
        if (avrg_eng_step[n_avrg_eng_step-1] > (background_eng*2)):# 是报警点
            alarm_sign = 1# 报警
