...
class RobotLoc:
    ...
    def set_r_t(self, odommsg):
        ...
        self.T = self.fro_T + np.array([[p.x + 0.3], [p.y + 0.4]])
        self.R_ori = quaternion_matrix((q.x, q.y, q.z, q.w))[0:2, 0:2]
        self.T_ori = np.array([[p.x + 0.3], [p.y + 0.4]])
        # odomfix = Odometry()
        odomfix = odommsg
...
def scan_cb(laser_msg):
    ...
    plt_data_b_x = static_map.data[0, :]
    plt_data_b_y = static_map.data[1, :]
    P_p_loop = P_p_scan
    while not rospy.is_shutdown():
