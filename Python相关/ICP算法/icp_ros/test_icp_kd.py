...
def static_map_get():
    ...
    try:
        ...
        # plt_data_a_x = [0,1,2,3]
        # plt_data_a_y = [3,2,1,0]
...
def closest_point_kd(kdtree, P):
    ...
    for i in range(len(dist_collect)):
        if abs(dist_collect[i]) <= ppp_dist:
...
def icp_map_process():
    ...
    while not rospy.is_shutdown():
        ...
        d_k_1 = 65535
        root = KD_node()
        dynamic_map = static_map.get_map(robot_loc.T, robot_loc.R)
        global plt_data_c_x, plt_data_c_y
        plt_data_c_x = dynamic_map[0, :]
        plt_data_c_y = dynamic_map[1, :]
        # print dynamic_map
        # plt_data_c_x = [1,2,3,4,5,6]
        # plt_data_c_y = [1,2,3,4,5,6]
        ...
        # d_k_list.append(d_k)
        # if len(d_k_list) >= 200:
        #     file_handler = open("/home/ros/dk.txt", 'w')
        #     file_handler.write(str(d_k_list))
        #     file_handler.close()
        #     print "!!!!!!!!!!!!!!!!!"
        if d_k_1 <= 0.010:
