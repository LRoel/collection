...
class MapData:
    ...
    def __init__(self, info, data):
        ...
        np_data[np.where(np_data != 100)] = 0
        self.grid_data = np_data.tolist()
...
class PlotFigure(wx.Frame):
    ...
    def onTimer(self, evt):
        ...
        self.user, = self.ax.plot(fix_point.point[0, :], fix_point.point[1, :], 'or')
        ...
        frame_ori = np.array([[xlim[0], ylim[0]], [xlim[0], ylim[1]], [xlim[1], ylim[1]], [xlim[1], ylim[0]],
                          [xlim[0], ylim[0]]]).transpose()
        T = np.array([[odom_msg.pose.pose.position.x], [odom_msg.pose.pose.position.y]])
        R = quaternion_matrix((odom_msg.pose.pose.orientation.x, odom_msg.pose.pose.orientation.y, odom_msg.pose.pose.orientation.z, odom_msg.pose.pose.orientation.w))[0:2, 0:2]
        frame = np.dot(R, frame_ori) + np.dot(T, np.ones((1, frame_ori.shape[1])))
        # delta = 0.3
        ...
        self.user, = self.ax.plot(grid_point.point[0, :], grid_point.point[1, :], 'og', label='2')
        # just draw the "animated" objects
        self.ax.draw_artist(
            self.user)
        # It is used to efficiently update Axes data (axis ticks, labels, etc are not updated)
...
def grid(fix_point, static_map):
    ...
    for laser_data in fix_point.point.transpose():
        ...
        #     d_1 = [65535] * 4
        #     # l
        #     if static_map.grid_data[int(grid_data[1] * static_map.width + grid_data[0] - 1)]:
        #         flag_1[0] = 1
        #         d_1[0] = (tmp_laser[1] - grid_data[1]) ** 2 + (tmp_laser[1] - grid_data[0] - 1) ** 2
        #     # r
        #     if static_map.grid_data[int(grid_data[1] * static_map.width + grid_data[0] + 1)]:
        #         flag_1[1] = 1
        #         d_1[1] = (tmp_laser[1] - grid_data[1]) ** 2 + (tmp_laser[1] - grid_data[0] + 1) ** 2
        #     # u
        #     if static_map.grid_data[int((grid_data[1] + 1) * static_map.width + grid_data[0])]:
        #         flag_1[2] = 1
        #         d_1[2] = (tmp_laser[1] - grid_data[1] + 1) ** 2 + (tmp_laser[1] - grid_data[0]) ** 2
        #     # d
        #     if static_map.grid_data[int((grid_data[1] - 1) * static_map.width + grid_data[0])]:
        #         flag_1[3] = 1
        #         d_1[3] = (tmp_laser[1] - grid_data[1] - 1) ** 2 + (tmp_laser[1] - grid_data[0]) ** 2
        #     if sum(flag_1) >= 1:
        #         #判定最近点
        #         d_min_index = d_1.index(min(d_1))
        #         if d_min_index
        #     else:
        #         flag_2 = [0] * 4
        #         # lu
        #         if static_map.grid_data[int((grid_data[1] + 1) * static_map.width + grid_data[0] - 1)]:
        #             flag_2[0] = 1
        #         # ru
        #         if static_map.grid_data[int((grid_data[1] + 1) * static_map.width + grid_data[0] + 1)]:
        #             flag_2[1] = 1
