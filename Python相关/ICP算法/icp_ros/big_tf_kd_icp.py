...
class RobotLoc:
    def __init__(self):
        ...
        self.R_map_odom_ready = np.eye(2)
        self.T_map_odom_ready = np.zeros((2, 1))
        self.lock_flag = False
        # self.T_map_odom = np.array([[-3.0], [-3.0]])

    def set_R_T_ready(self, R, T):
        self.R_map_odom_ready = R
        self.T_map_odom_ready = T

    def set_R_T(self):
        # self.R_map_odom = np.dot(self.R_map_odom, R)
        # self.T_map_odom += T
        R = self.R_map_odom_ready
        T = self.T_map_odom_ready
...
def judge_scan(laser_range, map_range):
    _laser_range = np.asarray(laser_range)
    _map_range = np.asarray(map_range)
    _judge_range = np.abs(_laser_range - _map_range)
    _judge_logical = _judge_range <= 1
    # np.logical_or(_judge_range <= 0.2, _judge_range > 50)
    if 1.0*np.sum(_judge_logical)/len(_judge_logical) >= 0.2:
        _laser_range[np.logical_not(_judge_logical)] = 10
        robot_loc.lock_flag = False
    else:
        robot_loc.lock_flag = True
        print "asdfghjkl"
        # print np.sum(_judge_logic)/len(_judge_logic)
    return _laser_range
...
def initialpose_cb(initialpose):
    _R_old, _T_old = robot_loc.map()
    q = initialpose.pose.pose.orientation
    p = initialpose.pose.pose.position
    _R_new = quaternion_matrix((q.x, q.y, q.z, q.w))[0:2, 0:2]
    _T_new = np.array([[p.x], [p.y + 0.5]])
    '''
    _R_new = R * _R_old
    _T_new = T + _T_old
    '''
    R = np.dot(_R_new, la.inv(_R_old))
    T = _T_new - _T_old
    robot_loc.set_R_T_ready(R, T)
    robot_loc.set_R_T()
    robot_loc.icp_fix()
