...
void WritePng(const PixelDataMatrix& mat, FileWriter* const file_writer) {
  ...
  for (int y = 0; y < mat.rows(); ++y) {
    for (int x = 0; x < mat.cols(); ++x) {
      ...
      /**
      * @brief 将'mat'作为一个令人愉快的PNG写入'filename'我们使用对数加权来衡量像素的饱和度.这里的基本思想是，墙壁（全高）完全饱和，但是像椅子和桌子这样的细节仍然很明显.
      */
  ...
  // the caller.
  /**
  * @brief TODO（hrapp）：cairo_image_surface_create_for_data在表面完成之前不拥有数据的所有权.一旦完成，cairo_surface_write_to_png失败，抱怨表面已经定稿.这使得很难将图像的所有权传回给呼叫者.
  */
