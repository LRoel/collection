...
PointCloud AdaptivelyVoxelFiltered(
  ...
  if (result.size() >= options.min_num_points()) {
    ...
    /**
    * @brief 使用'max_length'进行过滤导致足够密集的点云.
    */
    return result;
  }
  ...
  /**
  * @brief 搜索已知导致足够密集的点云的“low_length”.如果将边缘长度减小1e-2因子，我们放弃并使用完整的'point_cloud'是不够的.
  */
  for (float high_length = options.max_length();
    ...
    if (result.size() >= options.min_num_points()) {
      ...
      /**
      * @brief 二进制搜索找到适当的过滤量.'low_length'给出了一个足够密集的'result'，'high_length'没有.当边缘长度最多为10％时，我们停止.
      */
