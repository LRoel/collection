...
void Submaps::AddProbabilityGridToResponse(
  ...
  for (const Eigen::Array2i& xy_index :
    ...
    if (probability_grid.IsKnown(xy_index + offset)) {
      ...
      /**
      * @brief 我们想添加'delta'，但这不可能使用一个值和alpha.我们使用预乘α，所以当'delta'为正时，我们可以通过将'alpha'设置为零来添加它.如果是负数，我们将'value'设置为零，并使用'alpha'进行减法.当像素当前为白色时，这只是正确的，所以墙壁看起来太灰.尽管如此，这应该很难在视觉上被检测出来.
      */
      const int delta =
