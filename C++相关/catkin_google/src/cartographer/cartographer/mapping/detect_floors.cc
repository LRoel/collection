...
/**
* @brief 在作为不相交集林实现的轨迹中将层次分配给“跨度”的联合查找结构：https：// en.维基百科.org / wiki / Disjoint-set_data_structure＃Disjoint-set_forests TODO（hrapp）：我们在别的地方使用.拉出一个共同的地方.
*/
...
/**
* @brief 指标进入轨迹.节点'，所以索引.开始<= i <index.结束.
*/
struct Span {
...
/**
* @brief 在z中跳过轨迹.当当前节点的z从中位数z值的差异超过kLevelHeightMeters时，将启动一个新的跨度.
*/
...
/**
* @brief 如果'span'被认为是短的话，.e.自己不是有趣的，但应该在进入之前和之后折叠成水平.
*/
...
/**
* @brief 将具有相似中位数z值的所有“跨度”合并到同一级别.
*/
...
std::vector<Floor> FindFloors(const proto::Trajectory& trajectory,
  ...
  */
  ...
  for (size_t i = 0; i < spans.size(); ++i) {
    ...
    // into it.
    /**
    * @brief 如果我们在这个楼层上有一块长片，将这个短片合并到它里面.
    */
    ...
    /**
    * @brief 否则，将此短片添加到其前后的级别.在楼梯上可能有一些中间水平.
    */
    size_t index = i - 1;
    if (index < spans.size()) {
  ...
  for (auto& level : level_spans) {
    ...
    for (const auto& span : level.second) {
      if (!IsShort(trajectory, span)) {
        ...
        /**
        * @brief 为了找出这个楼层的中间高度，我们只关心保证在结构中的长块.这是一种启发式，省略中间（短）级别.
        */
