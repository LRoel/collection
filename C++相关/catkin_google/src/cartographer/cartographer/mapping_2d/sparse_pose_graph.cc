...
namespace cartographer {
	namespace mapping_2d {
		...
		void SparsePoseGraph::AddScanGPS(common::Time time, const transform::Rigid3d &tracking_to_pose,
		                                 const sensor::RangeData &range_data_in_pose, const transform::Rigid2d &pose,
		                                 const kalman_filter::Pose2DCovariance &covariance,
		                                 const mapping::Submaps *submaps, const mapping::Submap *matching_submap,
		                                 const std::vector<const mapping::Submap *> &insertion_submaps,
		                                 const Eigen::Vector3f &gps_pos, const Eigen::Vector3f &gps_var){
			const transform::Rigid3d optimized_pose(GetLocalToGlobalTransform(*submaps) *
			                                        transform::Embed3D(pose));
		...
		void SparsePoseGraph::ComputeConstraintsForScan(
			...
			for (const mapping::Submap* submap : insertion_submaps) {
				...
				/**
				* @brief 不变的协方差（submap < -  map）是一个翻译.
				*/
				const transform::Rigid2d constraint_transform =
						sparse_pose_graph::ComputeSubmapPose(*submap).inverse() * pose;
				Eigen::Matrix<double, 2, 1> gps_pose_2d;
				gps_pose_2d[0] = scan_data->gps_data.gpspose[0];
				gps_pose_2d[1] = scan_data->gps_data.gpspose[1];
				const transform::Rigid2d constraint_transform_gps(gps_pose_2d,scan_data->gps_data.gpspose[2]);
				Eigen::Matrix<double, 6, 6> GPS_var;
				GPS_var[0, 0] =  scan_data->gps_data.gpsvar[0];
				GPS_var[1, 1] =  scan_data->gps_data.gpsvar[1];
				GPS_var[2, 2] =  scan_data->gps_data.gpsvar[2];
				...
				constraints_.push_back(Constraint{
						...
						{transform::Embed3D(constraint_transform_gps),GPS_var},
			...
			if (finished_submap != nullptr) {
				...
				// old scans.
				/**
				* @brief 我们有一个新的完成的子图，所以我们研究添加旧的扫描的约束.
				*/
			...
			if (options_.optimize_every_n_scans() > 0 &&
				...
				/**
				* @brief 如果有'scan_queue_'已经有一些其他的线程会小心.
				*/
				if (scan_queue_ == nullptr) {
