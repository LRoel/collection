...
sensor::RangeData LocalTrajectoryBuilder::TransformAndFilterRangeData(
  ...
  /**
  * @brief 将任何回报降低到最小范围以下，并将超出最大范围的回报转换为错失.
  */
...
void LocalTrajectoryBuilder::ScanMatch(
  ...
  /**
  * @brief 在线相关扫描匹配器将细化Ceres扫描匹配器的初始估计.
  */
...
LocalTrajectoryBuilder::AddHorizontalRangeData(
  ...
  /**
  * @brief 如果我们没有使用IMU，请立即初始化IMU跟踪器.
  */
  ...
  if (imu_tracker_ == nullptr) {
    ...
    /**
    * @brief 直到我们用我们的第一个IMU消息初始化了IMU跟踪器，我们无法计算激光扫描仪的方向.
    */
  ...
  /**
  * @brief TODO（whess）：如果配置，喜欢IMU在odom方向？.
  */
  const transform::Rigid3d tracking_to_tracking_2d =
  ...
  if (!odometry_state_tracker_.empty()) {
    ...
    /**
    * @brief 我们添加了一个测距状态，使得扫描匹配的校正不会被我们得到的下一个测距数据消除.
    */
...
void LocalTrajectoryBuilder::AddImuData(
  ...
  /**
  * @brief 如果背包倾斜度超过20度，请记录警告.在这些情况下，2D SLAM很可能会失败.
  */
...
void LocalTrajectoryBuilder::AddOdometerData(
  ...
  if (imu_tracker_ == nullptr) {
    ...
    /**
    * @brief 在我们初始化IMU跟踪器之前，我们不想调用Predict（）.
    */
    LOG(INFO) << "ImuTracker not yet initialized.";
    return;
  }
...
void LocalTrajectoryBuilder::Predict(const common::Time time) {
  ...
  if (time_ > common::Time::min()) {
    ...
    */
    ...
    /**
    * @brief 使用当前的IMU跟踪滚轮和俯仰进行重力对准，并应用其偏航变化.
    */
