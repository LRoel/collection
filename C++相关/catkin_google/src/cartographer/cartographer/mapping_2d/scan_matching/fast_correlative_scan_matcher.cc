...
/**
* @brief 可以检索可以添加和稍后删除的值的集合，并且可以检索集合中当前值的最大值.
*/
...
class SlidingWindowMaximum {
 public:
  ...
  void RemoveValue(const float value) {
    ...
    // precomputation grid.
    /**
    * @brief DCHECK表现，因为这是为预计算网格中的每个值完成的.
    */
 ...
 private:
  ...
  /**
  * @brief 最前面的滑动窗口最大.然后在该值首次出现之后的剩余窗口的最大值等等.
  */
...
PrecomputationGrid::PrecomputationGrid(
  ...
  /**
  * @brief 首先，我们计算由x0 <= x <x0 + width定义的跨度中实现的每个（x0，y）的最大概率.
  */
  ...
  // score.
  /**
  * @brief 对于每个（x，y），我们计算从每个（x，y）开始的宽度x宽度区域的最大概率，并预先计算得出的得分界限.
  */
