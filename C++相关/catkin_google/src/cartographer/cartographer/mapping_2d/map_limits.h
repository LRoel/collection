...
// performance reasons.
/**
* @brief 定义网格图的限制.出于性能原因，此类必须保持内联.
*/
class MapLimits {
 public:
  ...
  /**
  * @brief 以米为单位返回单元格大小.所有单元格均为正方形，分辨率为单面长度.
  */
  ...
  // smaller coordinates.
  /**
  * @brief 返回极限的角落，我.e.，所有像素都具有坐标较小的位置.
  */
  ...
  */
  ...
  // false for Contains().
  /**
  * @brief 返回包含可能在映射之外的点（'x'，'y'）的单元格的索引，i.e.对于Contains（）返回false的负数或太大的索引.
  */
  Eigen::Array2i GetXYIndexOfCellContainingPoint(const double x,
    ...
    // rotate.
    /**
    * @brief 索引值是行主，左上角有Eigen :: Array2i :: Zero（），并包含（centered_max_x，centered_max_y）.我们需要翻转和旋转.
    */
  ...
  /**
  * @brief 返回true的ProbabilityGrid包含'xy_index'.
  */
  ...
  /**
  * @brief 计算包含原点的MapLimits，以及“轨迹”中的所有激光光线（返回和未命中）.
  */
  static MapLimits ComputeMapLimits(
    ...
    // discretization.
    /**
    * @brief 添加一些填充以确保离散后所有的光线仍然包含在地图中.
    */
