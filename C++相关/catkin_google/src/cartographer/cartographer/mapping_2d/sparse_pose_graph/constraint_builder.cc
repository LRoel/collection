...
void ConstraintBuilder::ComputeConstraint(
  ...
  /**
  * @brief 'constraint_transform'（i < -  j）由以下公式计算：
   -  j中的'filtered_point_cloud'， 
   - （map < -  j）的初始猜测'initial_pose'， 
   -  Match（）的结果'pose_estimate'（map < -  ComputeSubmapPose（）（map < -  i）.
  */
  ...
  // CSM estimate.
  /**
  * @brief 使用CSM估计作为初始姿势和以前的姿势.这样做的效果是，在没有更好的信息的情况下，我们更喜欢原始的CSM估计.
  */
