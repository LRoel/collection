...
// orientation.
/**
* @brief 将姿势转换为用于Ceres的3优化变量格式：x和y中的平移，后跟表示方向的旋转角度.
*/
...
/**
* @brief 将Ceres代表的姿势转换为transform :: Rigid2d姿势.
*/
			...
			void OptimizationProblem::Solve(
				...
				*/
				for (const Constraint& constraint : constraints) {
					...
					problem.AddResidualBlock(
							new ceres::AutoDiffCostFunction<SpaCostGPSFunction, 3, 3>(
									new SpaCostGPSFunction(constraint.gpspose)),
							// Only loop closure constraints should have a loss function.
							/**
							* @brief 只有循环闭合约束应该具有损失函数.
							*/
...
//							: nullptr,
				...
				// 'trajectory'.
				/**
				* @brief 'node_data_'中的姿势从多个轨迹交错（尽管来自给定轨迹的点是时间顺序的）.
				* 	   'last_pose_indices [轨迹]'是“轨迹”上最近的姿势的指标.
				*/
