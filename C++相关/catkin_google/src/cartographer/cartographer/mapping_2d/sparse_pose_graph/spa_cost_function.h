	...
	namespace mapping_2d {
		namespace sparse_pose_graph {
			...
			public:
				...
				/**
				* @brief 计算扫描到子图对齐'zbar_ij'与子图姿态'c_i'和扫描姿势'c_j'之间的差异，它们都在任意公共帧.
				*/
				template <typename T>
				...
				/**
				* @brief 计算由'sqrt_Lambda_ij'缩放的错误，将其存储在'e'.
				*/
				template <typename T>
			...
			class SpaCostGPSFunction {
			public:
				using Constraint = mapping::SparsePoseGraph::Constraint;

				explicit SpaCostGPSFunction(const Constraint::GPSPose& pose) : pose_(pose) {}
				...
				*/
				template <typename T>
				static std::array<T, 3> ComputeUnscaledError(
						const transform::Rigid2d& zbar_ij,
						const T* const c_j) {
					return {{T(zbar_ij.translation().x()) - c_j[0],
							        T(zbar_ij.translation().y()) - c_j[1],
							        common::NormalizeAngleDifference(T(zbar_ij.rotation().angle()) -
							                                         c_j[2])}};
				}
				...
				*/
				...
				static void ComputeScaledError(const Constraint::Pose& pose,
				                               ...
				                               T* const e) {
					std::array<T, 3> e_gj =
							ComputeUnscaledError(transform::Project2D(pose.zbar_ij), c_j);
					(Eigen::Map<Eigen::Matrix<T, 3, 1>>(e)) =
							kalman_filter::Project2D(pose.sqrt_Lambda_ij).cast<T>() *
							Eigen::Map<Eigen::Matrix<T, 3, 1>>(e_gj.data());
				}

				template <typename T>
				bool operator()(const T* const c_j, T* e) const {
					ComputeScaledError(pose_, c_j, e);
					return true;
				}

			private:
				const Constraint::GPSPose pose_;
			};
