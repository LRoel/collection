...
/**
* @brief 当一个LuaParameterDictionary被构造时，一个新的Lua状态（即.e.一个独立的Lua解释器）被解雇以评估Lua代码.该代码预期返回一个Lua表，其中包含作为参数字典的键/值对的键/值对.
*/
//
...
// table.
/**
* @brief 我们保留Lua解释器和堆栈上的表，并在Get *方法中引用它，而不是将其内容从Lua移动到C ++映射中，因为我们只能在Get *方法中知道表中期望的数据类型.
*/
//
...
// can read back.
/**
* @brief 以下一些方法使用以下符号记录当前堆栈：S：<bottom>.<top>通过Lua可以回读的引用版本替换堆栈顶部的字符串.
*/
...
// table.
/**
* @brief S：.字符串格式“％q”string将给定的“字典”设置为Lua注册表中“this”键的值.
*/
...
/**
* @brief 使用Lua字典中的给定“键”读取值，并将其推送到堆栈的顶部.
*/
template <typename T>
...
/**
* @brief 如果“key”位于Lua堆栈顶部的表中，则返回true.
*/
template <typename T>
...
/**
* @brief 在“L”的堆栈顶部迭代表的整数键，并逐个推送值.'pop_value'预计会弹出一个值并将其放入一个C ++容器中.
*/
...
std::unique_ptr<LuaParameterDictionary> LuaParameterDictionary::PopDictionary(
  ...
  /**
  * @brief 构造函数lua_xmove（）是该值，不需要弹出它.
  */
  CheckTableIsAtTopOfStack(L_);
  return value;
}
...
/**
* @brief Lua函数在当前的Lua上下文中运行一个脚本.将文件名作为唯一的参数.
*/
