...
struct ConstantYawQuaternionPlus {
  ...
  bool operator()(const T* x, const T* delta, T* x_plus_delta) const {
    ...
    /**
    * @brief 我们将“delta”应用于子映射帧的xy平面中的角轴旋转矢量.这样我们可以对齐重力，因为在地图框架中Z轴周围的旋转不会改变重力对齐，同时不允许与重力对齐无关的地图的随机旋转（i.e.我们不允许改变完整地图的“偏航”的步骤）.
    */
    ...
    */
    ...
    for (size_t j = 1; j < node_indices.size(); ++j) {
      ...
      if (j + 1 < node_indices.size()) {
        ...
        /**
        * @brief 'delta_velocity'是从第一个和第二个姿势之间的时间点到第二个和第三个姿势之间的时间点的速度变化.它是从IMU数据计算的，并且由于重力仍然包含一个增量.该向量的方向在第二姿势的IMU帧中.
        */
