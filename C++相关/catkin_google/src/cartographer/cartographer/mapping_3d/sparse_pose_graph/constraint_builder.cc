...
void ConstraintBuilder::ComputeConstraint(
  ...
  /**
  * @brief 'constraint_transform'（i < -  j）由以下公式计算： -  j中的'filtered_point_cloud'， - （map < -  j）的初始猜测'initial_pose'， -  Match（）的结果'pose_estimate'（map <.
  */
  ...
  if (match_full_submap) {
    ...
    if (submap_scan_matcher->fast_correlative_scan_matcher->Match(
      ...
      /**
      * @brief  -  submap-> pose（）（map < -  i）我们报告了一个成功的本地匹配.
      */
