...
/**
* @brief 这是一个类似于mapping_2d :: FastCorrelativeScanMatcher的3D分支和绑定算法的实现.
*/
...
struct Candidate {
  ...
  */
  ...
  /**
  * @brief 从单元索引中的初始姿态的线性偏移.对于较低分辨率的候选者，这是2 ^深度x 2 ^深度x 2 ^深度可能性块的最小偏移.
  */
...
class FastCorrelativeScanMatcher {
 public:
  ...
  // matcher score.
  /**
  * @brief 将'hybrid_grid'中的'rough_point_cloud'对齐为'initial_pose_estimate'.如果超过'min_score'（不包括平等）的分数是可能的，则返回true，并且使用结果更新'score'和'pose_estimate'.'fine_point_cloud'用于计算旋转扫描匹配器得分.
  */
  ...
  /**
  * @brief 在“hybrid_grid”中对齐“rough_point_cloud”，给定一个预期大致重力对齐的旋转.如果超过'min_score'（不包括平等）的分数是可能的，则返回true，并且使用结果更新'score'和'pose_estimate'.'fine_point_cloud'用于计算旋转扫描匹配器得分.
  */
