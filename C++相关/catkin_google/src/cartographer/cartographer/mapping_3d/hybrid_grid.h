...
// z-major index.
/**
* @brief 将每个维度的“索引”从0转换为2 ^“位”-1转换为平坦的z主索引.
*/
...
// from 0 to 2^'bits' - 1.
/**
* @brief 将平坦的z主要索引转换为3维索引，每个维度从0到2 ^“位”-1.
*/
...
/**
* @brief 在连续存储器中存储'ValueType'类型的'2 ^ kBits'x'2 ^ kBits'x'2 ^ kBits'体素的平坦网格.每个维度中的指数为0.
*/
...
class FlatGrid {
  ...
  */
  ...
  /**
  * @brief 返回存储在“index”的值，“index”的每个维度在0和grid_size（） -  1之间.
  */
...
// 'mutable_value()'.
/**
* @brief 由“WrappedGrid”类型的'2 ^ kBits'x'2 ^ kBits'x'2 ^ kBits'网格组成的网格.首先通过'mutable_value（）'构建包装网格.
*/
...
class NestedGrid {
  ...
  */
  ...
  /**
  * @brief 返回指向“index”值的指针，以允许更改它.如果需要，构建新的包装网格以包含该值.
  */
  ValueType* mutable_value(const Eigen::Array3i& index) {
    const Eigen::Array3i meta_index = GetMetaIndex(index);
 ...
 private:
  ...
  // 'index'.
  /**
  * @brief 返回包含'index'的元单元格的Eigen :: Array3i（meta）索引.
  */
...
/**
* @brief 最初由“WrappedGrid”类型组成的2×2x2格网格.首先通过'mutable_value（）'构建包装网格.如果需要，网格在每个维度上增长到两倍.指数范围（几乎）围绕原点对称，i.e.负指数是允许的.
*/
...
class DynamicGrid {
  ...
  */
  ValueType value(const Eigen::Array3i& index) const {
    ...
    /**
    * @brief 未签名的演员是通过3对比的shift_index进行性能检查.[xyz]> = 0和shif_index.[xyz] <grid_size.
    */
  ...
  /**
  * @brief 返回指向“index”值的指针，以允许更改它，动态增长DynamicGrid并根据需要构建新的WrappedGrids.
  */
  ValueType* mutable_value(const Eigen::Array3i& index) {
    const Eigen::Array3i shifted_index = index + (grid_size() >> 1);
    // The cast to unsigned is for performance to check with 3 comparisons
...
class HybridGridBase : public Grid<ValueType> {
 public:
  ...
  // 'resolution'.
  /**
  * @brief 围绕'origin'创建一个新的基于树的概率网格，它成为索引（0,0,0）上的单元格的中心，.每个体素具有边长度“分辨率”.
  */
  ...
  /**
  * @brief 返回包含'point'的单元格的索引.指数是识别单元格的整数向量，因此与原点的相对距离将舍入为下一个分辨率的倍数.
  */
  ...
  /**
  * @brief 返回一个八分位数（0，0，0），（1，0，0），.，（1,1,1）.
  */
...
// marker per voxel.
/**
* @brief 包含使用15位存储的概率值的网格，以及每个体素的更新标记.
*/
class HybridGrid : public HybridGridBase<uint16> {
 public:
  ...
  HybridGrid(const proto::HybridGrid& proto)
    ...
    for (int i = 0; i < proto.values_size(); ++i) {
      ...
      /**
      * @brief SetProbability为我们做了一些错误检查.
      */
  ...
  */
  ...
  /**
  * @brief 如果单元格尚未更新，则将调用ComputeLookupTableToApplyOdds（）时指定的“odds”应用于“index”单元格的概率.在调用StartUpdate（）之前，将忽略同一单元格的多个更新.如果单元格已更新，则返回true.
  */
  //
  ...
  */
  ...
  /**
  * @brief 如果指定的'index'的概率是已知的，则返回true.
  */
