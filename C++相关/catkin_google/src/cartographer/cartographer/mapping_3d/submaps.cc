...
/**
* @brief 我们在xy平面周围计算一个切片.“变换”应用于全局地图帧中的光线，以允许选择任意切片.
*/
...
// information.
/**
* @brief 过滤器的range_data'，只保留与原点不超过“max_range”距离的返回值.消除错误和反射率信息.
*/
...
Submaps::Submaps(const proto::SubmapsOptions& options)
  ...
  /**
  * @brief 我们总是想要有至少一个可以返回的可能性字段，并且将在没有更好选择的情况下在原点创建它.
  */
...
void Submaps::SubmapToProto(
  ...
  /**
  * @brief 通过“hybrid_grid”生成X射线视图，与全局地图帧中的xy平面对齐.
  */
