...
/**
* @brief 计算两个速度之间差异的代价.对于恒速模型，残差只是向量差.
*/
...
OptimizingLocalTrajectoryBuilder::AddRangefinderData(
  ...
  */
  ...
  if (batches_.empty()) {
    ...
    /**
    * @brief TODO（hrapp）：range_data_in_tracking中的NaN在哪里来自？.初始化到原点.
    */
    batches_.push_back(
  ...
  */
  ...
  if (odometer_data_.size() > 1) {
    ...
    for (size_t i = 1; i < batches_.size(); ++i) {
      ...
      // from the odometer.
      /**
      * @brief 如果我们有来自里程表的包围数据，则仅为此范围数据添加约束.
      */
...
OptimizingLocalTrajectoryBuilder::PredictState(const State& start_state,
  ...
  /**
  * @brief TODO（hrapp）：IntegrateImu也应该集成位置.
  */
