...
/**
* @brief 它扩展为3D中的子映射：每个扫描已经与一个或多个子映射匹配（为每个匹配添加约束），扫描和子图的两个姿势都将被优化.
*/
...
class SparsePoseGraph : public mapping::SparsePoseGraph {
 public:
  ...
  /**
  * @brief 在“时间”上添加一个新的“range_data_in_tracking”观察结果，稍后将优化“姿态”.'pose'是通过与'matching_submap'的扫描匹配确定的，扫描被插入到'insertion_submaps'.返回与GetTrajectoryNodes（）一起使用的轨迹节点向量中的索引.
  */
 ...
 private:
  struct SubmapState {
    ...
    */
    ...
    // which are finished.
    /**
    * @brief 是否在当前状态的后台线程中完成此子图.当它转换为true时，会尝试所有扫描以匹配此子图.同样，所有新的扫描都与完成的子图匹配.
    */
    bool finished = false;
  ...
  */
  ...
  /**
  * @brief 等待，直到我们赶上（我.e.没有什么正在等待安排），所有的计算都完成了.
  */
