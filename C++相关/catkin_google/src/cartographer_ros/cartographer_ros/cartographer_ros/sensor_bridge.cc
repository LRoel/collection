...
namespace cartographer_ros {
    ...
    void SensorBridge::HandleLaserScanMessage(
        ...
        HandleRangeGPSfinder(sensor_id, FromRos(msg->header.stamp), msg->header.frame_id, msg);
    }
    ...
    void SensorBridge::HandleRangeGPSfinder(const string &sensor_id, const ::cartographer::common::Time time,
                                            const string &frame_id, const sensor_msgs::LaserScan::ConstPtr &msg){
        carto::sensor::PointCloud ranges;
        carto::sensor::Data::GPS laserGPS;
        ranges = carto::sensor::ToPointCloud(ToCartographer(*msg));
        int tmp = sizeof(msg->ranges);
        laserGPS.gps_pose[0] = msg->intensities[tmp];
        laserGPS.gps_pose[1] = msg->intensities[tmp + 1];
        laserGPS.gps_pose[2] = msg->intensities[tmp + 2];
        laserGPS.gps_cov[0] = msg->intensities[tmp + 3];
        laserGPS.gps_cov[1] = msg->intensities[tmp + 4];
        laserGPS.gps_cov[2] = msg->intensities[tmp + 5];
        const auto sensor_to_tracking =
                tf_bridge_.LookupToTracking(time, CheckNoLeadingSlash(frame_id));
        if (sensor_to_tracking != nullptr) {
            trajectory_builder_->AddRangefinderData(
                    sensor_id, time, sensor_to_tracking->translation().cast<float>(),
                    carto::sensor::TransformPointCloud(ranges, sensor_to_tracking->cast<float>()),
                    laserGPS);
        }
    }
