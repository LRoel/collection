...
class AmclNode
{
  ...
  private:
    ...
    geometry_msgs::PoseWithCovarianceStamped tmp_map_odom_;
        ...
        first_reconfigure_call_(true)
{
  ...
  tf_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("/fuse/map_odom", 2, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      pf_vector_t cep_mean;
      double cep_var;
      pf_get_cep_stats(pf_, &cep_mean, &cep_var);
      ROS_INFO("cep_mean: %6.3f %6.3f %6.3f cep_var: %6.3f",
               cep_mean.v[0], cep_mean.v[1], cep_mean.v[2], cep_var);
      ROS_INFO("simple_count: %d  cluster_count: %d",
               set->sample_count, set->cluster_count);
      ...
      else
      {
    	...
    	tf::transformStampedTFToMsg(tmp_tf_stamped, tf_map_odom_);
    	tmp_map_odom_.header = tf_map_odom_.header;
    	tmp_map_odom_.pose.pose.position.x = tf_map_odom_.transform.translation.x;
    	tmp_map_odom_.pose.pose.position.y = tf_map_odom_.transform.translation.y;
    	tmp_map_odom_.pose.pose.position.z = tf_map_odom_.transform.translation.z;
    	tmp_map_odom_.pose.pose.orientation.x = tf_map_odom_.transform.rotation.x;
    	tmp_map_odom_.pose.pose.orientation.y = tf_map_odom_.transform.rotation.y;
    	tmp_map_odom_.pose.pose.orientation.z = tf_map_odom_.transform.rotation.z;
    	tmp_map_odom_.pose.pose.orientation.w = tf_map_odom_.transform.rotation.w;
    	tmp_map_odom_.pose.covariance = p.pose.covariance;
    	tf_pub_.publish(tmp_map_odom_);
    	// this->tfb_->sendTransform(tmp_tf_stamped);
    	sent_first_transform_ = true;
      }
    }
