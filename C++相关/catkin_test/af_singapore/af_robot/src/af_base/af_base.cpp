...
unsigned char AFBase::check_sum(unsigned char *buffer, int start, int end)
{
    unsigned char sum = 0;
...
: fd_(0)
{
    ...
    charging_sub_ = nh_.subscribe("charging_enabled", 100, &AFBase::chargingCallback, this);
...
void AFBase::readThread()
{
    ...
    while (ros::ok())
    {
        ...
        if (read(fd_, &recv_c, 1) != 1)
            continue;
        std::cout <<index << " 读取字符index(" << index << "): " << std::hex << int(recv_c) <<std::endl;
        msg.data[index] = recv_c;
        if (index == 0)
        {
            if (recv_c == '\x86' || recv_c == '\x87')
            {
		ROS_INFO("first byte: %x", recv_c);
                index = 1;
            }
        }
        else if (index == bit_len-1)
        {
            ...
            {
                std::cout << "校验和不匹配, 校验和: "<< (cs & 0xFF) << std::endl;
		...
		std::cout << std::endl;
              ...
              if ((0xFF & msg.data[0]) == (0xFF &'\x86')) {
		ROS_INFO("published val topic");
                ...
                index = 0;
              } else if ((0xFF & msg.data[0]) == (0xFF & '\x87')) {
		index = 0;
              }
            }
        }
