...
def encode_cb(encodemsg):
    global yaw,yaw_sum,robot_x,robot_y

    m_length = 0.590
    PulseToDistance = math.pi * 0.1524 / 2000
    ...
    if gps_get_flag == 1:
        ...
        yaw_sum += m_rotation
        gps_get_flag == 0
    m_tranX = (thetaL + thetaR) * PulseToDistance * math.cos(yaw_sum) / 2.0
    m_tranY = (thetaL + thetaR) * PulseToDistance * math.sin(yaw_sum) / 2.0

    # robot_trans = tf.TransformBroadcaster()
    # print robot_x + m_tranX
    # robot_trans.sendTransform((robot_x + m_tranX, robot_y + m_tranY, 0),
    #                  quaternion_from_euler(0, 0, yaw_sum),
    #                  rospy.Time.now(),
    #                  "base_footprint",
    #                  "odom")
...
def odom_cb(odommsg):
    # print "odom cb"
    global odom_msg, odom_yaw_angle, odom_yaw_old
...
def gps_cb(gpsMsg):
    ...
    global odom_msg,robot_odom,gps_recovery_flag,latitude_0,longitude_0,seq
    ...
    if (abs(sum_gps_angle) <= abs(sum_odom_angle) + 5) and gpsMsg.NSV1 > 4:
    ...
    robot_odom.twist.twist.linear.x = robot_v
...
def nav_filter():
    ...
    rospy.Subscriber('/gps', Novatel, gps_cb)
