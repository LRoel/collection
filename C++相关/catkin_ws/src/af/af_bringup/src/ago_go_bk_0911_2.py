...
def nav_check(g_x, g_y, g_yaw, a_x, a_y, a_yaw, o_x, o_y, o_yaw, c_xy, c_yaw):
    if (g_x - o_x) ** 2 + (g_y - o_y) ** 2 <= c_xy ** 2:
    ...
    if (g_yaw - o_yaw) ** 2 <= c_yaw ** 2:
        if (a_yaw - o_yaw) ** 2 <= c_yaw ** 2:
            nav_yaw_flag = 3
        else:
            nav_yaw_flag = 2
    else:
...
def gps_callback(gpsmsg):
    ...
    global out_msg, pub_amcl, amcl_flag, gps_flag
    ...
    if not init_flag:
        ...
        if N >= 10:
            ...
            if (gps_var_x + gps_var_y) / 2 <= 0.05 ** 2:
                ...
                amcl_init_pose.pose.pose.position.x = mean_x
                amcl_init_pose.pose.pose.position.y = mean_y
                amcl_init_pose.pose.pose.position.z = 0

                q = quaternion_from_euler(0, 0, gps_yaw)
...
def odom_callback(odommsg):
    ...
    global constraint_r, constraint_yaw, amcl_flag, gps_flag, odom_fix ,odom_filter
    ...
    if init_flag:
        ...
        (nav_flag, nav_yaw_flag) = nav_check(odom_gps_x, odom_gps_y, odom_gps_yaw, odom_amcl_x, odom_amcl_y, odom_amcl_yaw, out_msg[0], out_msg[1], out_msg[2], constraint_r, constraint_yaw)
...
def ago_go():
    # print "nav filter"
    global tf_listener, pub_filter, pub_fix, pub_amcl
