...
constraint_r = 0.65
constraint_yaw = 0.135
constraint_v_yaw = 0.385
...
filename = '/home/ros/' + str(datetime.datetime.now()) + '_data.txt'
...
def cmd_vel_state(cmd_bool):
    global cmd_state,pub_cmd
    cmd_state.data = cmd_bool
    pub_cmd.publish(cmd_state)


def v_heading(Ve, Vn, Heading):

    Vne = math.sqrt(Vn * Vn + Ve * Ve)
    if Vne > 0.3:
        if Ve < 0:
            out = (math.acos(Vn / Vne))+math.pi/2
        else:
            out = (2 * math.pi - math.acos(Vn / Vne))+math.pi/2
        out = do_yaw(out)  
    else:
        out = Heading
    return out
...
def nav_check(g_x, g_y, a_x, a_y, o_x, o_y, c_xy, g_state):
    if (g_state == 1 or g_state == 3) and ((g_x - o_x) ** 2 + (g_y - o_y) ** 2 <= c_xy ** 2 or g_state == 1):
        if (a_x - o_x) ** 2 + (a_y - o_y) ** 2 <= c_xy ** 2:
            nav_flag = 3
        else:
            nav_flag = 2
    else:
...
def nav_yaw_check(g_yaw, a_yaw, v_yaw, e_yaw, g_state, a_state, c_yaw, c_v_yaw):
    global output
    global n,plt_data6,plt_data7, v_msg
    global nav_yaw_array
    nav_yaw_array = [0]*3
    if (g_state == 1 or g_state == 2) and (abs(g_yaw - e_yaw) <= c_yaw or g_state == 1 or g_state == 2) and ((abs(v_yaw - g_yaw) <= c_v_yaw) or (abs(2 * math.pi - v_yaw - g_yaw) <= c_v_yaw)):
        nav_yaw_array[0] = 1

    if a_state <= 0.0000001 and abs(a_yaw - e_yaw) <= c_yaw:
        nav_yaw_array[1] = 1

        
    #print "__________________"
    #print v_yaw*180/math.pi, g_yaw*180/math.pi
    
    # plt_data4.append(a_yaw)
    # plt_data6.append(v_yaw)
    # plt_data7.append(g_yaw)
    
    # if n > 3000:
    #     fig = plt.figure(1)
    #     ax = fig.add_subplot(111)
    #     plot1 = ax.plot(plt_data6, label='v_yaw')
    #     plot2 = ax.plot(plt_data7, label='gps_yaw')
    #     plot3 = ax.plot(plt_data4, label='a_yaw')
    #     plt.legend(loc='lower right')
    #     plt.title('yaw')
    #     #plt.show()
    #     n = 0

    s = str(g_yaw)+','+str(a_yaw)+','+str(v_yaw)+','+str(e_yaw)+','+str(v_msg[0])+','+str(v_msg[1])+','+str(g_state)+','+str(a_state)+'\r\n'
    output.write(s)
    s_debug = "g:%.3f  a:%.3f  e:%.3f  g:%d  a:%f\r\n"%(g_yaw,a_yaw,e_yaw,g_state,a_state)

    print nav_yaw_array
    print s_debug
    print "----------------------------------------"
    return nav_yaw_array
...
def gps_callback(gpsmsg):
    ...
    global encode_delta_x,encode_delta_y,encode_delta_yaw, v_msg
    global init_flag, init_msg, amcl_flag, gps_yaw_flag, m_sumQ, v_yaw
    global out_msg, pub_amcl, amcl_flag, gps_flag, g_state, n
    global odom_amcl_yaw
    global amcl_gps_flag
    global nav_yaw_array
    ...
    v_msg[0] = gpsmsg.twist.twist.linear.x
    v_msg[1] = gpsmsg.twist.twist.linear.y
    v_yaw = v_heading(v_msg[0], v_msg[1], gps_msg[2])

    g_state = gpsmsg.twist.twist.linear.z
    ...
    #    print N
        if N >= 10:
            ...
            if len(gps_init_x) > 10:
                gps_init_x.pop(0)
                gps_init_y.pop(0)
            #print (gps_var_x + gps_var_y) / 2
            print (g_state == 1 or g_state == 2)
            if (gps_var_x + gps_var_y) / 2 <= 0.05 ** 2 and (g_state == 1):
                init_flag = 1
                init_msg[0] = mean_x
                init_msg[1] = mean_y
                init_msg[2] = gps_yaw
                ...
                amcl_update(mean_x, mean_y, gps_msg[2])
                amcl_flag = 1
                gps_flag = 1
    ...
    if gps_flag == 1:
        ...
        encode_delta_yaw = []
        gps_msg_old = list(gps_msg)
        # amcl_update(gps_msg[0], gps_msg[1], odom_amcl_yaw)
        if nav_yaw_array[0] and not nav_yaw_array[1]:
            print "amcl_update(gps_msg[0], gps_msg[1], gps_msg[2])"
            # amcl_update(gps_msg[0], gps_msg[1], gps_msg[2])
            amcl_update(out_msg[0], out_msg[1], out_msg[2])


def amcl_callback(amclmsg):

    global amcl_msg, amcl_msg_old, amcl_msg_delta, amcl_flag
    global encode_delta_x_amcl,encode_delta_y_amcl,encode_delta_yaw_amcl, a_state

    ##############################################################
    a_state = amclmsg.pose.covariance[35]
...
def odom_callback(odommsg):
    ...
    global constraint_r, constraint_yaw, constraint_v_yaw, amcl_flag, gps_flag, odom_fix ,odom_filter
    global n, m_tranX, m_tranY, m_rotation
    global odom_v_yaw, v_yaw
    global g_state, a_state
    global pub_amcl
    global amcl_gps_flag

    cmd_vel_state(init_flag)
    if init_flag:
        ...
        # (odom_roll, odom_pitch, odom_yaw) = euler_from_quaternion(odom_quat)
        ...
        odom_v_yaw = do_yaw(v_yaw + sum(encode_delta_yaw))
        #print odom_gps_yaw
        ...
        nav_flag = nav_check(odom_gps_x, odom_gps_y, odom_amcl_x, odom_amcl_y, out_msg[0], out_msg[1], constraint_r, g_state)
        ...
        global nav_yaw_flag
        nav_yaw_flag = nav_yaw_check(odom_gps_yaw, odom_amcl_yaw, v_yaw, m_sumQ, g_state, a_state, constraint_yaw, constraint_v_yaw)
        amcl_gps_flag = nav_yaw_check(odom_gps_yaw, odom_amcl_yaw, v_yaw, m_sumQ, g_state, a_state, constraint_yaw/1.2, constraint_v_yaw)
        # nav_yaw_flag[0] = nav_yaw_flag[1] = 0

        if nav_yaw_flag[0] and nav_yaw_flag[1]:
            out_msg[2] = odom_amcl_yaw
        elif nav_yaw_flag[0] and not nav_yaw_flag[1]:
            out_msg[2] = odom_gps_yaw
        elif nav_yaw_flag[1] and not nav_yaw_flag[0]:
            out_msg[2] = odom_amcl_yaw
        else:
            out_msg[2] = odom_out_yaw
...
def ago_go():
    # print "nav filter"
    global tf_listener, pub_filter, pub_fix, pub_amcl, pub_cmd
